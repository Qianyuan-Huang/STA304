...
data5 %>% filter(!is.na(Count_)) %>% 
  ggplot(aes(x= ReportedYear, fill = Subtype)) + geom_bar() + theme_classic()+
  geom_text(stat = 'count',aes(label =..count.., vjust =-6))+
...
data5 %>% filter(!is.na(Count_)) %>% 
  ggplot(aes(x= Subtype, fill = AgeGroup)) + geom_bar() + theme_classic()+
  geom_text(stat = 'count',aes(label =..count.., vjust =0))+
  labs(x = "Crime Category", y = "Age Group", title = "Figure 2: Age Group and Crime Category")
```
